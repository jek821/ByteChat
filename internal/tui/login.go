...
func newLoginModel(auth client.AuthClient) loginModel {
	...
	return loginModel{auth: auth, inputs: inputs}
}
...
func (m loginModel) submit() tea.Cmd {
	...
	auth := m.auth
	return func() tea.Msg {
		creds, err := auth.Login(username, password)
		if err != nil {
			return authErrorMsg{err: err}
		}
		return loginSuccessMsg{creds: creds}
	}
}
